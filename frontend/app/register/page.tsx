  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (registerError) {
        ...
        if (registerError.includes('wallet_address') && (registerError.includes('duplicate') || registerError.includes('unique'))) {
          // User already exists, redirect to dashboard
          router.push('/dashboard')
          return
        }
        // Check if error is due to duplicate username
        if (registerError.includes('username') && (registerError.includes('duplicate') || registerError.includes('unique'))) {
      ...
      <div className="flex items-center justify-center p-4 min-h-[calc(100vh-80px)]">
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold pixel-text text-[#2D3748] mb-2">Welcome to CakePay</h1>
          ...
          {!walletAddress ? (
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              <div>
                <label htmlFor="wallet-address" className="block text-sm font-bold pixel-text text-[#2D3748] mb-2">
                  Wallet Address
                </label>
              ...
              <div>
                <label htmlFor="username" className="block text-sm font-bold pixel-text text-[#2D3748] mb-2">
                  Username *
                </label>
                <input
                  id="username"
                ...
                />
                <p className="text-xs text-[#4A5568] mt-1">
                  This will help others find you when adding members to groups (minimum 2 characters)
                </p>
              </div>

              <div>
                <label htmlFor="avatar-url" className="block text-sm font-bold pixel-text text-[#2D3748] mb-2">
                  Avatar URL (optional)
                </label>
                <input
                  id="avatar-url"
